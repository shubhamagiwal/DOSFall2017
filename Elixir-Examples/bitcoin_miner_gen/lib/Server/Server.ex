...
use GenServer
    ...
    def start_server_node(tuple) do
       ...
       IO.inspect Node.spawn(Node.self,fn -> BitcoinMinerGen.Server.start(elem(tuple,1)) end);       
    end
    ...
    def start(k) do
        ...
        GenServer.call(Server,:get_work_load);
        GenServer.call(Server,:get_work_load);
    ...
    def handle_cast(:sendRequest,state_tuple)do
      ...
      def handle_cast({:got,msg},state_tuple)do
        IO.puts msg
        {:noreply,state_tuple}
    end
